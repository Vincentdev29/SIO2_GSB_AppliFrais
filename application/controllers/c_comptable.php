	...
	*/
	public function _remap($action, $params = array())
	{
		...
		if (!$this->authentif->estConnecte())
		{
			...
			$data = array();
			$this->templates->load('t_comptable', 'v_connexion', $data);
		}
		else
		{
			...
			{	// TODO : conrôler que l'obtention des données postées ne rend pas d'erreurs
				...
				// vérification des champs données
				// vérification de la date
				$verifDate = false;

				// vérification du montant
				$verifMontant = $this->load->estReelPositif($uneLigne['montant']);

				if($verifDate)// vérifie si la date est valide
				{
					if($verifMontant)// vérifie si le montant est valide
					{
						$this->a_visiteur->ajouteFrais($idVisiteur, $mois, $uneLigne);
					}
					else// affiche un message d'erreur si montant invalide
					{

					}
				}
				else// affiche un message d'erreur si date invalide
				{
